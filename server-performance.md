...
**Magnitude の二乗比較:** 距離を比較するだけであれば平方根計算を省略できる。`.Magnitude` プロパティは内部で sqrt を計算するため、`Vector3:Dot()` で二乗距離を直接求めて比較する。

```lua
-- BAD: 平方根を計算してから比較
if (posA - posB).Magnitude < 50 then

-- GOOD: Dot積で二乗距離を求め、閾値も二乗で比較（sqrtなし）
local diff = posA - posB
if diff:Dot(diff) < 50 * 50 then
```

大量のオブジェクトに対する距離チェックでは、この手法で sqrt の計算コストを完全に排除できる。
...
**制限事項:**
- コンパイル時間とメモリ消費が増加する
- 体験あたりのネイティブコード総量にメモリ上限がある
- 関数内の1コードブロックあたり64K命令が上限
- 1関数あたり32K内部ブロックが上限
- 1スクリプトあたり100万命令が上限
- 型注釈（特にVector3等）を付けるとより効果的に最適化される
- `debug.dumpcodesize()` でネイティブコードサイズを確認できる
...
公式ドキュメントがスロットリング制限を明記しているのはクライアント→サーバー方向（`FireServer`）のみである。サーバー→クライアント方向（`FireClient` / `FireAllClients`）にも実務上は帯域制約があるが、具体的な数値は公式に公開されていない。いずれの方向でも、高頻度の送信は帯域を圧迫するため間引きが推奨される。
...
2026年4月以降、サーバーのデフォルト予算が大幅に引き上げられた。旧制限は `× 10` だったが、新制限では `× 40` となっている。`DataStoreService:SetRateLimitForRequestType()` で個別に調整も可能である。

| 操作 | 旧予算（〜2026年3月） | 新予算（2026年4月〜） |
|---|---|---|
| GetAsync | 60 + numPlayers × 10 | 60 + numPlayers × 40 |
| SetAsync / IncrementAsync / UpdateAsync / RemoveAsync | 60 + numPlayers × 10 | 60 + numPlayers × 40 |

**OrderedDataStore（サーバーあたり/分）:**

| 操作 | 旧予算 | 新予算（2026年4月〜） |
|---|---|---|
| GetSortedAsync | 5 + numPlayers × 2 | 5 + numPlayers × 2（変更なし） |
| SetAsync / IncrementAsync / RemoveAsync | 30 + numPlayers × 5 | 30 + numPlayers × 5（変更なし） |
| GetAsync（OrderedRead） | — | 60 + numPlayers × 40 |

`numPlayers` はそのサーバーのアクティブプレイヤー数。`UpdateAsync` は読み取りと書き込みの**両方の予算を1回で消費する**。

現在の残り予算は `DataStoreService:GetRequestBudgetForRequestType()` で実行時に確認できる。
